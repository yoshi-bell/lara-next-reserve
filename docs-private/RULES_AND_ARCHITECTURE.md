...
*   **詳細な実装ルール:** コーディング規約やコメント規則については **`docs-private/CODING_GUIDELINES.md`** を参照すること。

---

# lara-next-reserve ルールとアーキテクチャ (Rules & Architecture)
...
*   **型安全性の担保:** **OpenAPI (Scribe)** を介してバックエンドの仕様をフロントエンドの型定義 (**openapi-typescript**) に自動同期させ、手動管理による不整合を防ぐ。
...
*   **物理削除:** 予約キャンセルは `delete` を使用し、物理削除を行う（キャンセル後の再予約を容易にするため）。

### バリデーション
*   **多層防御 (Defense in Depth):**
    *   **Backend:** `FormRequest` クラスでビジネスロジック（重複、定員）を含む厳密なチェックを行う。
    *   **Frontend:** `Zod` 等のスキーマ検証ライブラリを用い、入力値だけでなく **APIレスポンスの型安全性** もランタイムで保証する。
