...
**現在のアクティブタスク:** ステップ 5-1 店舗機能 > **店舗一覧ページのUI構築**
**ステータス:** Imageコンポーネントの `sizes` プロパティ追加に着手する前の状態。
...
- [ ] **店舗一覧ページのUI構築**
    - [ ] **実装:**
        - [ ] `shops/page.tsx` で、`useShops` フックを使いデータを取得する。
        - [ ] 取得した店舗データをカード形式で表示するUIを構築する（店舗名、画像、エリア名、ジャンル名）。
        - [ ] `Area` と `Genre` の名前を表示するため、バックエンドの `ShopController@index` を `with(['area', 'genre'])` でリレーションをEager Loadingするように修正する。
        - [ ] `useShops` フックの型定義を、`Area` と `Genre` のリレーションを含むように更新する。
    - [ ] **動作確認:**
        - [ ] ブラウザで `/shops` にアクセスし、店舗一覧が意図通りに表示されるか確認する。
