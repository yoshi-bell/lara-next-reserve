...
**現在のアクティブタスク:** ステップ 5-2 予約・お気に入り機能 > **お気に入り機能の実装**
**ステータス:** 店舗機能（一覧・検索・詳細）が完了し、お気に入り機能の実装準備中。
...
- [x] **店舗一覧ページのUI構築**
    - [x] **実装:**
        ...
        - [x] `Image` コンポーネントに `sizes` プロパティを追加し、パフォーマンス警告を解消する。
    - [x] **動作確認:**
...
- [x] **検索機能の実装**
    - [x] **実装:**
        - [x] エリア、ジャンル、キーワードによる絞り込みUIを実装する。
        - [x] 検索条件の変更に応じて、店舗一覧を非同期で更新するロジックを実装する (`useShops` の引数または `mutate` を活用)。
    - [x] **動作確認:**
        - [x] 各検索条件で店舗が正しくフィルタリングされることを確認する。
    - [x] **プロセス記録:**
        - [x] `DEV_LOG.md`, `AGENT_RECOVERY_MANUAL.md` を更新する。

- [x] **店舗詳細ページの実装**
    - [x] **実装:**
        - [x] `src/hooks/useShopDetail.ts` を作成する。
        - [x] `/detail/[shop_id]` のルートを作成し、UIを構築する。
        - [x] 店舗一覧から詳細ページへのリンクを設定する。
    - [x] **動作確認:**
        - [x] 店舗一覧から詳細ページに正しく遷移でき、情報が表示されることを確認する。
    - [x] **プロセス記録:**
        - [x] `DEV_LOG.md`, `AGENT_RECOVERY_MANUAL.md` を更新する。
...
- [x] **お気に入り機能の実装**
    - [x] **バックエンド実装:**
        - [x] `FavoriteController` を作成し、お気に入りの登録/解除を行う `store`/`destroy` メソッドを実装する。
        - [x] `routes/api.php` に `/shops/{shop}/favorite` のようなルートを定義する (`auth:sanctum` ミドルウェアで保護)。
    - [x] **フロントエンド実装:**
        - [x] ハートアイコンクリックで、`axios` を使ってお気に入り登録/解除APIをコールするロジックを実装する。
        - [x] 画面遷移なしでハートアイコンの表示状態を更新する。
    - [x] **動作確認:**
        - [x] ログイン状態で、お気に入り登録・解除が正しく動作することを確認する。
        - [x] データベースの `favorites` テーブルにレコードが正しく作成・削除されることを確認する。
    - [x] **プロセス記録:**
        - [x] `DEV_LOG.md`, `AGENT_RECOVERY_MANUAL.md` を更新する。
