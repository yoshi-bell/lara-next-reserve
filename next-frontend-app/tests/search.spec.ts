...
test.describe("店舗検索機能", () => {
    ...
    areas.forEach((targetArea) => {
        test(`エリア「${targetArea.name}」で絞り込みができること`, async ({
            page,
        }) => {
            // エリア選択
            const areaSelect = page
                .locator("select")
                .filter({ hasText: "All area" });
            ...
            await expect(
                page.getByText(targetArea.representative),
            ).toBeVisible();
            ...
            for (const otherArea of otherAreas) {
                await expect(
                    page.getByText(otherArea.representative),
                ).not.toBeVisible();
            }
        });
    });
    ...
    genres.forEach((targetGenre) => {
        test(`ジャンル「${targetGenre.name}」で絞り込みができること`, async ({
            page,
        }) => {
            const genreSelect = page
                .locator("select")
                .filter({ hasText: "All genre" });
            ...
            await expect(
                page.getByText(targetGenre.representative),
            ).toBeVisible();

            // 2. 選択していない他のジャンルの代表店舗が表示されていないこと
            const otherGenres = genres.filter(
                (g) => g.name !== targetGenre.name,
            );
            for (const otherGenre of otherGenres) {
                await expect(
                    page.getByText(otherGenre.representative),
                ).not.toBeVisible();
            }
        });
    });
    ...
    test("エリア・ジャンル・キーワードの複合検索ができること", async ({
        page,
    }) => {
        // 条件設定: 東京都 × 寿司 × キーワード"仙"
        ...
        await page
            .locator("select")
            .filter({ hasText: "All area" })
            .selectOption({ label: "東京都" });
        await page
            .locator("select")
            .filter({ hasText: "All genre" })
            .selectOption({ label: "寿司" });
